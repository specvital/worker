---
title: NaCl SecretBox 암호화
description: OAuth 토큰 암호화를 위한 NaCl SecretBox 선택 결정
---

# ADR-13: NaCl SecretBox 암호화

> 🇺🇸 [English Version](/en/adr/core/13-nacl-secretbox-encryption.md)
...
2. **Worker 서비스**: GitHub API 접근 시 복호화
...
| 서비스 간 공유   | web/worker에서 동일 코드베이스 사용   |
...
3. **이식성**
   - `golang.org/x/crypto`의 순수 Go 구현
   - CGO 의존성 없음
   - 크로스 플랫폼 빌드 가능
...
- [pkg/crypto/doc.go](https://github.com/specvital/core/blob/main/pkg/crypto/doc.go) - 사용 예제 포함 패키지 문서
