...
func TestGenerateSpecViewUseCase_Options(t *testing.T) {
	...
	t.Run("batch config option", func(t *testing.T) {
		...
		uc := NewGenerateSpecViewUseCase(
			...
			&mockAIProvider{},
			"gemini-2.5-flash",
			WithBatchConfig(batchCfg),
		)

		if uc.config.BatchConfig.BatchThreshold != 500 {
			t.Errorf("expected batch threshold 500, got %d", uc.config.BatchConfig.BatchThreshold)
		}
		if !uc.config.BatchConfig.UseBatchAPI {
			t.Error("expected use batch API to be true")
		}
	})
}

func TestShouldUseBatchAPI(t *testing.T) {
	t.Run("returns true when batch enabled and test count meets threshold", func(t *testing.T) {
		uc := NewGenerateSpecViewUseCase(
			&mockRepository{},
			&mockAIProvider{},
			"gemini-2.5-flash",
		...
		if !uc.ShouldUseBatchAPI(100) {
			t.Error("expected ShouldUseBatchAPI to return true when count equals threshold")
		}
		if !uc.ShouldUseBatchAPI(200) {
			t.Error("expected ShouldUseBatchAPI to return true when count exceeds threshold")
		}
	})

	t.Run("returns false when test count below threshold", func(t *testing.T) {
		uc := NewGenerateSpecViewUseCase(
			&mockRepository{},
			&mockAIProvider{},
			"gemini-2.5-flash",
		...
		if uc.ShouldUseBatchAPI(99) {
			t.Error("expected ShouldUseBatchAPI to return false when count is below threshold")
		}
	})

	t.Run("returns false when batch API disabled", func(t *testing.T) {
		uc := NewGenerateSpecViewUseCase(
			&mockRepository{},
			&mockAIProvider{},
			"gemini-2.5-flash",
		...
		if uc.ShouldUseBatchAPI(200) {
			t.Error("expected ShouldUseBatchAPI to return false when batch API is disabled")
		}
	})

	t.Run("returns false with default config", func(t *testing.T) {
		uc := NewGenerateSpecViewUseCase(
			&mockRepository{},
			&mockAIProvider{},
			"gemini-2.5-flash",
		...
		if uc.ShouldUseBatchAPI(10000) {
			t.Error("expected ShouldUseBatchAPI to return false with default config")
		}
	})
}

func TestExecutePhase2And3FromBatch(t *testing.T) {
	t.Run("success - processes phase 2 and 3 from batch result", func(t *testing.T) {
		var savedDoc *specview.SpecDocument
		repo := &mockRepository{
			getAnalysisContextFn: func(ctx context.Context, analysisID string) (*specview.AnalysisContext, error) {
				return &specview.AnalysisContext{Host: "github.com", Owner: "test-owner", Repo: "test-repo"}, nil
			},
			getTestDataByAnalysisIDFn: func(ctx context.Context, analysisID string) ([]specview.FileInfo, error) {
				return newTestFiles(), nil
			},
			saveDocumentFn: func(ctx context.Context, doc *specview.SpecDocument) error {
				savedDoc = doc
				doc.ID = "doc-001"
		...
		result, err := uc.ExecutePhase2And3FromBatch(context.Background(), req, phase1Output)
		if err != nil {
	...
	t.Run("error - nil phase1 output", func(t *testing.T) {
		repo := &mockRepository{
			getAnalysisContextFn: func(ctx context.Context, analysisID string) (*specview.AnalysisContext, error) {
				return &specview.AnalysisContext{Host: "github.com", Owner: "test-owner", Repo: "test-repo"}, nil
			},
		...
		_, err := uc.ExecutePhase2And3FromBatch(context.Background(), req, nil)
		if err == nil {
			t.Fatal("expected error for nil phase1 output")
		}
		if !strings.Contains(err.Error(), "phase 1 output is required") {
			t.Errorf("expected error about phase 1 output, got: %v", err)
		}
	})
	...
	t.Run("error - invalid request", func(t *testing.T) {
		...
		_, err := uc.ExecutePhase2And3FromBatch(context.Background(), req, newPhase1Output())
		if err == nil {
			t.Fatal("expected error for invalid request")
		}
	})

	t.Run("error - no test files found", func(t *testing.T) {
		repo := &mockRepository{
			getAnalysisContextFn: func(ctx context.Context, analysisID string) (*specview.AnalysisContext, error) {
				return &specview.AnalysisContext{Host: "github.com", Owner: "test-owner", Repo: "test-repo"}, nil
			},
		...
		_, err := uc.ExecutePhase2And3FromBatch(context.Background(), req, newPhase1Output())
		if err == nil {
			t.Fatal("expected error for no test files")
		}
	})

	t.Run("error - phase 2 AI failure", func(t *testing.T) {
		repo := &mockRepository{
			getAnalysisContextFn: func(ctx context.Context, analysisID string) (*specview.AnalysisContext, error) {
				return &specview.AnalysisContext{Host: "github.com", Owner: "test-owner", Repo: "test-repo"}, nil
			},
		...
		aiProvider := &mockAIProvider{
			convertTestNamesFn: func(ctx context.Context, input specview.Phase2Input) (*specview.Phase2Output, *specview.TokenUsage, error) {
				return nil, nil, errors.New("AI provider error")
			},
		}
		...
		_, err := uc.ExecutePhase2And3FromBatch(context.Background(), req, newPhase1Output())
		if err == nil {
			t.Fatal("expected error for phase 2 AI failure")
		}
		if !errors.Is(err, ErrAIProcessingFailed) {
			t.Errorf("expected error to wrap ErrAIProcessingFailed, got: %v", err)
		}
	})

	t.Run("error - save document failure", func(t *testing.T) {
		repo := &mockRepository{
			getAnalysisContextFn: func(ctx context.Context, analysisID string) (*specview.AnalysisContext, error) {
				return &specview.AnalysisContext{Host: "github.com", Owner: "test-owner", Repo: "test-repo"}, nil
			},
		...
		_, err := uc.ExecutePhase2And3FromBatch(context.Background(), req, newPhase1Output())
		if err == nil {
			t.Fatal("expected error for save document failure")
		}
		if !errors.Is(err, ErrSaveFailed) {
			t.Errorf("expected error to wrap ErrSaveFailed, got: %v", err)
		}
	})
}
